...
MatchResultList JobMatching::search(const string *skillSet, int skillCount, bool matchAll)
{
    if (matchMode == FIND_RESUME)
    {
        ...
        case ARRAY:
            ...
            {
                ResumeArray res = resumeArray->linearSearchBySkills(skillSet, skillCount, matchAll);

                MatchResultList results;
                for (int i = 0; i < res.getSize(); i++)
                {
                    Resume r = res.getResume(i);
                    cout << "Resume ID: " << r.id << endl;
                }
                return results;
            }
            else if (searchAlgo == BINARY)
            {
                ResumeArray res = resumeArray->binarySearchBySkills(skillSet, skillCount, matchAll);
    ...
    else if (matchMode == FIND_JOB)
    {
        ...
        case ARRAY:
            if (searchAlgo == LINEAR)
                if (searchAlgo == LINEAR)
                {
                    JobArray res = jobArray->linearSearchBySkills(skillSet, skillCount, matchAll);

                    MatchResultList results;
                    for (int i = 0; i < res.getSize(); i++)
                    {
                        Job r = res.getJob(i);
                        cout << "Job ID: " << r.id << endl;
                    }
                    return results;
                }
                else if (searchAlgo == BINARY)
                {
                    JobArray res = jobArray->binarySearchBySkills(skillSet, skillCount, matchAll);
